...
export function deriveTimelineCategory(node: GraphNode): TimelineCategory {
  const sourceKind = (node.sourceKind ?? "").toLowerCase();
  const evidence = (node.evidenceTags ?? []).map((tag) => tag.toLowerCase());
  const contested =
    sourceKind.includes("contested") ||
    sourceKind.includes("speculation") ||
    evidence.some((tag) => tag.includes("contested") || tag.includes("speculation"));
  if (contested) return "speculation-contested";
  ...
  if (node.entityType === "Event" && node.archetypalResonance) return "myth-in-time";
