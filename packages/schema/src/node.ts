...
export const graphNodeSchema = z.strictObject({
  ...
  contentRevision: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).nullable(),
  seedSchemaVersion: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).nullable(),
  ...
  qlSchemaVersion: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).nullable(),
