...
vi.mock("@research-canvas/desktop-api", async (importOriginal) => {
  ...
  return {
    ...
    createWorkspaceServices: () => ({
      ...
      // Shell navigation must cross successful persistence ports. Do not let
      // this unit fixture accidentally call the live browser bridge or rely
      // on navigation ignoring a failed layout flush.
      loadAppTabs: async () => ({ tabs: [], activeTabId: null }),
      saveAppTabs: async () => undefined,
      flushCanvasLayout: async () => true,
      persistConstellationDocument: async () => undefined,
      listPendingNodeDocumentSyncs: async () => [],
      ...
      selectProject: projectSpies.selectProject.mockImplementation(
        async ({ projectId }: { projectId: string }) => ({
          projectId,
          profileScope: projectId === "root" ? "bootstrapping" : `project:${projectId}`,
          rootType: "directory",
        }),
      ),
      loadConstellationDocument: async ({ databasePath, constellationId }: { databasePath: string; constellationId: string }) => ({
        ...
        workingRoot: constellationId === "root" ? "/workspace" : `/home/${constellationId}`,
        constellation: {
          id: constellationId,
          displayName: constellationId === "root" ? "Root Archetypal Field" : "Project A",
    ...
    <MemoryRouter initialEntries={initialEntries}>
      <Routes>
        <Route
          ...
          element={<CanvasWorkspaceProvider><Shell /></CanvasWorkspaceProvider>}
        />
        <Route path="*" element={<CanvasWorkspaceProvider><Shell /></CanvasWorkspaceProvider>} />
      </Routes>
    </MemoryRouter>,
  );
}
...
function seededNode(id: string, title: string): CanvasNode {
  return {
    id, graphNodeId: id, canvasId: CANVAS_ID, title,
    position: { x: 0, y: 0 }, size: { width: 160, height: 80 }, summary: "",
    sequenceCaption: null, sequenceViewport: null, type: "note", content: "", tags: [],
...
function seededResourceNode(id: string, title: string): CanvasNode {
  return {
    id, graphNodeId: id, canvasId: CANVAS_ID, title,
    position: { x: 0, y: 0 }, size: { width: 160, height: 80 }, summary: "",
    sequenceCaption: null, sequenceViewport: null, type: "resource", resourceKind: "markdown",
    absolutePath: "/tmp/fake.md", relativePath: "fake.md", mimeType: "text/markdown", fileFingerprint: "fp",
    createdAt: "2026-06-28T00:00:00.000Z", updatedAt: "2026-06-28T00:00:00.000Z",
  } as CanvasNode;
}
...
function seededPortalNode(id: string, title: string, targetCanvasId: string): CanvasNode {
  return {
    id, graphNodeId: id, canvasId: CANVAS_ID, title,
    position: { x: 0, y: 0 }, size: { width: 180, height: 96 }, summary: "Nested constellation",
    sequenceCaption: null, sequenceViewport: null, type: "portal", targetCanvasId, constellationKind: "ql-unit",
    createdAt: "2026-06-28T00:00:00.000Z", updatedAt: "2026-06-28T00:00:00.000Z",
  } as CanvasNode;
}

function FakeWorkspaceProvider({ children, nodes, canvases = {} }: {
  children: ReactNode; nodes: CanvasNode[]; canvases?: Record<string, CanvasNode[]>;
}) {
  const store = useMemo(() => {
  ...
  const value = useMemo(() => ({
    store, annotationStore, isHydrated: true, errorMessage: null, canvasId: CANVAS_ID,
    workspaceId: "sqlite:/canonical/workspace.sqlite", tabManager,
    activeSurfaceId: activeTab?.surfaceId ?? "canvas", tabs, activeTabId, activeTab,
    ...
    updateTabState: (state: SurfaceTabState) => {
      if (activeTabId) tabManager.getState().updateState(activeTabId, state);
    },
    canvasTabs: [], activeCanvasTabId: null, activeCanvasViewport: null,
    constellationId: "11111111-1111-4111-8111-111111111111", databasePath: "/canonical/workspace.sqlite",
    activeConstellation: null, activeConstellationId: "11111111-1111-4111-8111-111111111111",
    activeProjectId: "11111111-1111-4111-8111-111111111111", activeProfileScope: "bootstrapping",
    selectProject: vi.fn().mockResolvedValue(undefined),
    resolveOrCreateHome: vi.fn().mockResolvedValue({ homePath: "/home", projects: [] }), createProject: vi.fn(),
    constellations: [], entries: [], resourceRoots: [], workingRoot: null, repoRoot: "/workspace",
    selectedEntryId, selectedEdgeId, selectedNodeId, selectNode, selectEdge: setSelectedEdgeId, selectEntry: setSelectedEntryId,
    selectConstellation: vi.fn(), openCanvas, addEdge: vi.fn(),
    createNoteNode: vi.fn().mockResolvedValue(undefined), createGroupNode: vi.fn().mockResolvedValue(undefined),
    addResourceNode: vi.fn().mockResolvedValue(undefined), addResourceNodeFromAbsolutePath: vi.fn().mockResolvedValue(undefined),
    deleteEdge: vi.fn(), deleteNode: vi.fn(), duplicateNode: vi.fn().mockResolvedValue(undefined),
    attachResourceRoot: vi.fn().mockResolvedValue(undefined), detachResourceRoot: vi.fn().mockResolvedValue(undefined),
    listDirectories: vi.fn().mockResolvedValue([]), searchConstellation: vi.fn().mockResolvedValue([]),
    listSavedSequences: vi.fn().mockResolvedValue([]), createSavedSequence: vi.fn(), updateSavedSequence: vi.fn(),
    deleteSavedSequence: vi.fn().mockResolvedValue(undefined), resizeNode: vi.fn(), updateNodeContent: vi.fn(),
    setNodeThumbnailFromAbsolutePath: vi.fn().mockResolvedValue(undefined), updateNodeStyle: vi.fn(), updateNodeTags: vi.fn(),
    updateNodeTimelineCard: vi.fn(), flyToNode: vi.fn(), flyToEdge: vi.fn(), registerFlyToNode: vi.fn(), registerFlyToEdge: vi.fn(),
    captureViewport: () => ({ x: 0, y: 0, zoom: 1 }), registerCaptureViewport: vi.fn(),
    transport: createWorkspaceServices(), contentLinkingActions: {},
...
function renderShellWithNodes(nodes: CanvasNode[], canvases?: Record<string, CanvasNode[]>, initialEntries?: string[]) {
  return render(<MemoryRouter initialEntries={initialEntries}>
    <FakeWorkspaceProvider nodes={nodes} canvases={canvases}><Shell /></FakeWorkspaceProvider>
  </MemoryRouter>);
}

beforeEach(() => { projectSpies.selectProject.mockClear(); });
...
describe("Shell frame", () => {
  ...
  it("opens a portal's target canvas on double-click instead of opening the reader panel", async () => {
    ...
    const childNode = { ...seededNode("child-a", "Nested Child"), canvasId: targetCanvasId } as CanvasNode;
    ...
    await waitFor(() => expect(document.querySelector('.react-flow__node[data-id="child-a"]')).not.toBeNull());
    expect(screen.queryByTestId("reader-pane")).not.toBeInTheDocument();
  });
  ...
  it("route change selects the project and activates a surface owned by that project", async () => {
    renderShell(["/project/proj-a/surface/timeline/constellation/proj-a"]);
    await waitFor(() => expect(projectSpies.selectProject).toHaveBeenCalledWith(
