...
/// The full gate. Validation rejections are NOT hard errors: they produce a
/// fetch record with the failing `validation` flags so the reason is visible
/// to the agent (the CLI prints it back into the tmux session) and to the
/// frontend. Hard errors (unreadable source, missing database, import I/O)
/// are returned as `Err`.
pub fn ingest_fetched_asset_at(
    ...
    // Idempotent re-ingest: the same session + source URL + byte hash already
    // landed as an accepted record — return it instead of duplicating bytes or
    // a street-view image. Rejected records never match, so a corrected
    ...
    let validation = FetchValidation {
        ...
        size_ok: !bytes.is_empty() && (bytes.len() as u64) <= cap_bytes,
    ...
    let (artifact_path, street_view_image_id, redaction_status) = if all_ok {
        ...
        let status = if !request.redaction_regions.is_empty() {
            ...
            let output = apply_region_redaction(&media_root, &registered)
                .map_err(|error| error.to_string())?;
            sv_repo
