-- T10 (ticket #39): Canvas relationship kinds are sourced from the canonical
-- domain vocabulary instead of a hardcoded UI list. The vocabulary gained
-- RELATES_TO, CONTRADICTS, ARCHETYPE_EXPRESSES_AT, and SEQUENCE_NEXT.
-- SQLite cannot alter a CHECK constraint in place, so rebuild the local
-- projection atomically. The shared vocabulary in
-- relationship_vocabulary.rs is the authority; the placeholder is substituted
-- at migration time so the three boundaries cannot drift.
...
DROP INDEX IF EXISTS idx_graph_relationship_tombstone;

ALTER TABLE graph_relationship RENAME TO graph_relationship_0037_legacy;
...
SELECT
    ...
    sync_state, relationship_revision, remote_revision, created_at, updated_at,
    is_tombstone
FROM graph_relationship_0037_legacy;

DROP TABLE graph_relationship_0037_legacy;
