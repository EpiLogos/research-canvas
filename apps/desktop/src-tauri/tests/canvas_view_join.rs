...
fn load_canvas_view_is_layout_authoritative() {
    ...
    let synced = support::block_on(repo.create_node(NewGraphNode {
        graph_node_id: None,
        entity_type: "Event".into(), title: format!("Synced {run_id}"), body: "[]".into(),
    ...
    LayoutRepository::new(db.connection()).upsert_node_layout(&NodeLayoutRecord {
        graph_node_id: synced.graph_node_id.clone(), canvas_id: canvas_id.clone(),
    ...
    LayoutRepository::new(db.connection()).upsert_node_layout(&NodeLayoutRecord {
        graph_node_id: unsynced_id.clone(), canvas_id: canvas_id.clone(),
        position_x: 10.0, position_y: 20.0, width: 240.0, height: 160.0,
        style_json: sidecar_json.to_string(), created_at: now(), updated_at: now(),
    }).unwrap();
    ...
    LayoutRepository::new(db.connection()).upsert_node_layout(&NodeLayoutRecord {
        graph_node_id: unsynced_resource_id.clone(), canvas_id: canvas_id.clone(),
        position_x: 30.0, position_y: 40.0, width: 240.0, height: 160.0,
        style_json: resource_sidecar_json.to_string(), created_at: now(), updated_at: now(),
    }).unwrap();
    ...
    assert_eq!(view.nodes.len(), 3, "all three layout rows are returned regardless of Neo4j sync state");

    let synced_join = view.nodes.iter().find(|j| j.node.graph_node_id == synced.graph_node_id).unwrap();
    ...
    let unsynced_join = view.nodes.iter().find(|j| j.node.graph_node_id == unsynced_id).unwrap();
    assert_eq!(unsynced_join.node.title, format!("Unsynced Note {run_id}"), "title synthesized from sidecar");
    assert_eq!(unsynced_join.node.entity_type, "Work", "note sidecar maps to Work, matching entityTypeForNodeType");
    assert!(!unsynced_join.node.is_temporal, "synthesized nodes are not temporal");
    assert_eq!(unsynced_join.layout.position_x, 10.0);

    let unsynced_resource_join = view.nodes.iter().find(|j| j.node.graph_node_id == unsynced_resource_id).unwrap();
    assert_eq!(unsynced_resource_join.node.title, format!("Unsynced Resource {run_id}"));
    assert_eq!(unsynced_resource_join.node.entity_type, "Source", "resource sidecar maps to Source, matching entityTypeForNodeType");
    ...
    assert!(tl.nodes.iter().any(|j| j.node.graph_node_id == synced.graph_node_id));
    assert!(!tl.nodes.iter().any(|j| j.node.graph_node_id == unsynced_id));
    assert!(!tl.nodes.iter().any(|j| j.node.graph_node_id == unsynced_resource_id));

    support::block_on(async {
        graph.run_on(&database, query("MATCH (n {graph_node_id: $id}) DETACH DELETE n")
            .param("id", synced.graph_node_id)).await.expect("cleanup");
    });
}
