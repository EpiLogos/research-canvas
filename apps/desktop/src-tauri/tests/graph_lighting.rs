...
fn archetypal_lighting_returns_datable_instances() {
    ...
    let lit =
        support::block_on(repo.archetypal_lighting(&operator.graph_node_id)).expect("lighting");
    ...
    let inverse =
        support::block_on(repo.resonances_for_instance(&event.graph_node_id)).expect("inverse");
    assert!(inverse
        .iter()
        .any(|li| li.node.graph_node_id == operator.graph_node_id));
